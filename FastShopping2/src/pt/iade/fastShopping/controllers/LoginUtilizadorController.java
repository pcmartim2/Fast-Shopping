...
public class LoginUtilizadorController {
	...
	private void Login() {
		...
		if (!nome_Utilizador.getText().isEmpty()) {
			...
			if (DBConnector.verificarUtilizador(nome_Utilizador.getText())) {
				nome_UtilizadorLabel = nome_Utilizador.getText();
				//Vai converter o nome do utilizador em id
				IdUser = DBConnector.getIdUtilizador(nome_Utilizador.getText());
				if (nome_Utilizador.getText().equalsIgnoreCase("admin")) {
					WindowManager.openAdminWindow();
				}
				// login do proprietario da loja
				else if (DBConnector.verificarDonoLoja(nome_Utilizador.getText())) {
					lojaID = DBConnector.getLojaId(nome_Utilizador.getText());
				...
				DBConnector.addUtilizador(nome_Utilizador.getText());
				Login();
			}
		}
	}
