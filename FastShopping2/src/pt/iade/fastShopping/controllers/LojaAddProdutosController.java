...
public class LojaAddProdutosController {
	...
	private void initialize() {
		...
		DBConnector.getCategoriasProdutos(categoriaProduto);
    ...
    void addProduto(ActionEvent event) {
		if (imagemProduto.getImage() != null && !nome_Produto.getText().isEmpty() && preco_Produto.getValue() != null && !categoriaProduto.getSelectionModel().isEmpty() ) {
			...
			try {
				...
				baos.close();
				DBConnector.addProduto(imageInByte, nome_Produto.getText(), categoriaProduto.getSelectionModel().getSelectedItem(), String.valueOf(preco_Produto.getValue().floatValue()), LoginUtilizadorController.lojaID);
			} catch (IOException e) {
				e.printStackTrace();
			}
