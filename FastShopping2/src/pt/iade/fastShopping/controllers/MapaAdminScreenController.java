...
public class MapaAdminScreenController {
	...
	private void initialize() {
		...
		DBConnector.loadLojasMapa(root);
		
		
		// Vai adicionar o tipo de lojas no comboBox
		DBConnector.loadEstilosLojas(estilo_Loja);
		...
		if (imagemLoja.getImage() != null && !nome_Loja.getText().isEmpty() && !proprietario_Loja.getText().isEmpty()
			...
			if (selecionado) {
				...
				try {
					...
					baos.close();
					DBConnector.addLoja(nome_Loja.getText(), imageInByte, coordX+"#"+coordY, proprietario_Loja.getText(), estilo_Loja.getValue());
				} catch (IOException e) {
					e.printStackTrace();
				}
