...
public class SidebarLojaController {
	...
	private void initialize() throws IOException {
		...
		Image img = new Image(new ByteArrayInputStream(DBConnector.getImagemLoja(MapaScreenController.idLoja)));
		...
		if (DBConnector.getFavoritoLoja(id_Loja, id_User)) {
			fav.setVisible(true);
	...
	void addFavorito(MouseEvent event) {
		...
		if (event.getButton() == MouseButton.PRIMARY) {
			...
			DBConnector.addFavoritoUtilizador(id_User, MapaScreenController.idLoja);
			unfav.setVisible(false);
			fav.setVisible(true);
		}
	}
	...
	void removeFavorito(MouseEvent event) {
		...
		if (event.getButton() == MouseButton.PRIMARY) {
			...
			DBConnector.removeFavoritoUtilizador(id_User, MapaScreenController.idLoja);
			unfav.setVisible(true);
			fav.setVisible(false);
		}
	}
